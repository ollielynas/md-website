...
function load_md2(file) {
  if (file == null) {
    return;
  }

  // collapsed = ";".join(collapsed.split(";").filter((a) => !file.includes(a.replaceAll("\\", "/"))));
...
var files = "";
console.log(
  "this is decompressed",
  load_gzip("tree.txt").then((data) => {
    files = data;
    let file_list = files.replaceAll("\r", "").split("\n");
    collapsed = data.replace("\n",";");
